...
const EmployeeTableData = () => {
  ...
  const handleSearch = (e) => {
    ...
    if (query === "") {
      setFilteredEmployees(employees); // Reset the filter if the query is empty
    } else {
      const queryNumber = parseInt(query, 10); // Convert query to a number

      const filtered = employees.filter((employee) =>
        employee.employeeNumber.toString().includes(queryNumber.toString())
      );
